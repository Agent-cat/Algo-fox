...
export async function generateAnimationPlanAction(language: string, code: string, problemStatement?: string) {
...
${problemStatement ? `Problem Statement: ${problemStatement}` : ""}`;
        ...
        try {
            ...
            try {
                ...
                // 1. Escape raw newlines inside string literals
                cleanedText = cleanedText.replace(/"([^"\\]|\\.)*"/g, (match) => {
                    return match.replace(/\n/g, "\\n").replace(/\r/g, "\\r");
                });
                
                // 2. If truncated (unclosed braces/brackets), try to close them
                const openBraces = (cleanedText.match(/{/g) || []).length;
                const closeBraces = (cleanedText.match(/}/g) || []).length;
                const openBrackets = (cleanedText.match(/\[/g) || []).length;
                const closeBrackets = (cleanedText.match(/]/g) || []).length;
                
                // Remove trailing incomplete values (e.g., hanging comma or partial key)
                ...
                console.log("Recovered truncated JSON response successfully");
            } catch (e2: any) {
                console.error("Failed to parse JSON response:", text.substring(0, 500));
                return { success: false, error: "AI response was too large or malformed. Try shorter code." };
            }
        }
        ...
        return { success: false, error: e.message || "Failed to generate animation plan" };
    }
}
