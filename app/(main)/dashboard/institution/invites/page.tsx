...
export default function InvitesPage() {
  ...
  // Form state
  const [role, setRole] = useState<"TEACHER" | "CONTEST_MANAGER" | "STUDENT">(
    "TEACHER"
  );
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-2">
            Create and manage access links for your staff.
          </p>
        </div>
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 items-end">
          ...
          <button
            ...
            disabled={isCreating}
            className="h-11.5 w-full bg-gray-900 dark:bg-white text-white dark:text-black font-bold rounded-xl hover:opacity-90 transition-opacity disabled:opacity-50 flex items-center justify-center gap-2"
          >
            {isCreating ? (
            ...
            <div className="space-y-4">
              ...
              {!invite.isActive && (
                <div className="absolute inset-0 bg-white/50 dark:bg-black/50 backdrop-blur-[1px] rounded-2xl flex items-center justify-center">
                  <span className="px-4 py-2 bg-black text-white rounded-lg text-xs font-bold uppercase tracking-widest shadow-xl">
                    Deactivated
                  </span>
                </div>
              )}
      ...
      {invites.length === 0 && !isLoading && (
        <div className="text-center py-24 bg-gray-50 dark:bg-[#141414]/50 border border-dashed border-gray-200 dark:border-[#262626] rounded-3xl">
          ...
          <p className="text-sm text-gray-500 mt-2">
            Create your first invite link to start adding members.
          </p>
        </div>
      )}
    </div>
