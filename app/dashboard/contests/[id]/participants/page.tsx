...
export default function ContestParticipantsPage() {
    ...
    const getStatusBadge = (p: Participant) => {
        ...
        if (p.tempBlockedUntil && new Date(p.tempBlockedUntil) > new Date()) {
            return <span className="px-2 py-1 bg-orange-100 text-orange-700 text-xs font-medium rounded-full flex items-center gap-1">
        ...
        if (p.isFlagged) {
            return <span className="px-2 py-1 bg-yellow-100 text-yellow-700 text-xs font-medium rounded-full flex items-center gap-1">
        ...
        return <span className="px-2 py-1 bg-gray-100 text-gray-700 text-xs font-medium rounded-full">
            Active
        </span>;
    };
            ...
            <div className="bg-white border-b border-gray-200 sticky top-0 z-10">
                <div className="max-w-6xl mx-auto px-4 py-4">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-xl font-bold text-gray-900">Contest Participants</h1>
                            <p className="text-sm text-gray-500">View violations and manage blocked participants</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="max-w-6xl mx-auto px-4 py-6">
                ...
                <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                    {loading ? (
                        ...
                        <div className="p-8 text-center text-gray-500">No participants yet</div>
                    ) : (
                                            ...
                                            <div className="pt-4">
                                                ...
                                                {p.violations.length === 0 ? (
                                                    ...
                                                    <div className="space-y-2">
                                                        {p.violations.map((v) => (
                                                            <div key={v.id} className="flex items-center justify-between p-2 bg-white rounded-lg border border-gray-200">
                                                                <div className="flex items-center gap-2">
                                                                    <span className="px-2 py-0.5 bg-red-100 text-red-700 text-xs rounded font-medium">
                                                                        {v.type.replace(/_/g, ' ')}
                                                                    </span>
                                                                    {v.message && (
                                                                        <span className="text-sm text-gray-600">{v.message}</span>
                                                                    )}
                                                                </div>
