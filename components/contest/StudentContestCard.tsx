...
import { Trophy, Calendar, Users, Globe, School, ArrowRight, Timer, Clock, Play } from "lucide-react";
                ...
                <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                        ...
                        <span className={`text-xs font-bold uppercase tracking-wider ${
                            ...
                            isPast ? 'text-gray-500' : 'text-blue-600 dark:text-blue-500'
                        }`}>
                    ...
                    <div className="flex items-center gap-1.5 text-xs font-medium text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-[#1a1a1a] px-2.5 py-1 rounded-full border border-gray-100 dark:border-[#262626]">
                        <VisibilityIcon className="w-3 h-3" />
                         <span className="truncate max-w-[100px]">{getVisibilityInfo().label}</span>
                    </div>
                </div>
                    ...
                    <div className="grid grid-cols-2 gap-y-3 gap-x-4 p-4 bg-gray-50 dark:bg-[#1a1a1a]/50 rounded-xl border border-gray-100 dark:border-[#262626]">
                        ...
                        {isLive && timeLeft && (
                             <div className="col-span-2 flex items-center justify-between pt-2 border-t border-gray-200 dark:border-[#262626] mt-1">
                                <span className="text-[10px] uppercase font-bold text-orange-600 dark:text-orange-500">Time Remaining</span>
                                <span className="font-mono text-sm font-bold text-orange-600 dark:text-orange-500">{timeLeft}</span>
                            </div>
                        )}
                        {!isLive && !isPast && timeLeft && (
                            <div className="col-span-2 flex items-center justify-between pt-2 border-t border-gray-200 dark:border-[#262626] mt-1">
                                <span className="text-[10px] uppercase font-bold text-blue-600 dark:text-blue-500">Starts In</span>
                                <span className="font-mono text-sm font-bold text-blue-600 dark:text-blue-500">{timeLeft}</span>
                            </div>
                        )}
                    </div>
