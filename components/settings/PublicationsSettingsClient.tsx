...
export function PublicationsSettingsClient({ user, readonly = false }: PublicationsSettingsClientProps) {
    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-10">
            <div>
                <h1 className="text-2xl font-bold font-mono text-gray-900 dark:text-gray-100">Publications</h1>
                <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">
                    Manage your papers, books, and articles.
                </p>
            </div>
                                ...
                                <div className="flex items-start justify-between">
                                    <div className="space-y-1">
                                        <div className="flex items-center gap-2">
                                            <h3 className="font-bold text-gray-900 dark:text-gray-100 text-lg">{pub.title}</h3>
                                            <AlertCircle className="w-4 h-4 text-red-500" />
                                        </div>
                                        <div className="text-sm font-medium text-gray-500 dark:text-gray-400">
                                            {pub.publisher} | {formatDate(pub.publicationDate)}
                                        </div>
                                        {pub.publicationUrl && (
                                            <div className="text-sm text-orange-500 hover:text-orange-600 transition-colors">
                                                <a href={pub.publicationUrl} target="_blank" rel="noopener noreferrer">View Publication</a>
                                            </div>
                                        )}
                                    </div>
                                    {!readonly && (
