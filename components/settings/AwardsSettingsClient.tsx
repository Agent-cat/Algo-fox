...
interface AwardsSettingsClientProps {
    user: any;
    readonly?: boolean;
}

export function AwardsSettingsClient({ user, readonly = false }: AwardsSettingsClientProps) {
    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-10">
            <div>
                <h1 className="text-2xl font-bold font-mono text-gray-900 dark:text-gray-100">Awards & Recognitions</h1>
                <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">
                    Manage your awards, honors, and recognitions.
                </p>
            </div>
            ...
            <div className="space-y-6 pt-8 border-t-2 border-gray-200 dark:border-[#333]">
                ...
                {awards.length === 0 ? (
                    <div className="text-sm text-gray-500 dark:text-gray-400">
                        You have not added any awards yet.
                    </div>
                ) : (
                                ...
                                <div className="flex items-start justify-between">
                                    <div className="space-y-1">
                                        <div className="flex items-center gap-2">
                                            <h3 className="font-bold text-gray-900 dark:text-gray-100 text-lg">{award.positionTitle}</h3>
                                            <AlertCircle className="w-4 h-4 text-red-500" />
                                        </div>
                                        <div className="text-sm font-medium text-gray-500 dark:text-gray-400">
                                            {formatDate(award.issuedDate)} | {award.issuer}
                                        </div>
                                    </div>
                                    {!readonly && (
